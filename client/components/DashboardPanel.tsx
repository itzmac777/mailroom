                ...
                {filteredEmails.map((email) => {
                  ...
                  return (
                    <button
                      ...
                      className={`grid w-full grid-cols-[auto_1fr_auto] gap-3 p-5 text-left transition-colors ${active ? "bg-wash" : "bg-white hover:bg-[#fbfaf7]"}`}
                    >
                      <span className={`mt-1 h-2.5 w-2.5 rounded-full ${active ? "bg-cta" : "bg-[#f05a28]"}`} aria-hidden="true" />
                      <span className="min-w-0">
                        <span className="flex items-center justify-between gap-3">
                          <strong className="truncate text-sm text-ink">{senderName(email.from)}</strong>
                        </span>
                        <span className="mt-1 block truncate text-[15px] font-extrabold text-ink">{email.subject || "(No subject)"}</span>
                        <span className="mt-1 block truncate text-sm text-muted">{senderAddress(email.from)}</span>
                      </span>
                      <span className="whitespace-nowrap text-xs font-bold text-muted">{formatMessageDate(email.date)}</span>
                    </button>
                  );
                })}
              </div>
            ...
            {selectedEmail ? (
              <article className="mx-auto max-w-[780px]">
                <div className="border-b border-line pb-6">
                  <div className="mb-5 inline-flex items-center gap-2 border border-line bg-[#fbfaf7] px-3 py-2 text-xs font-extrabold uppercase tracking-[0.12em] text-muted">
                    <span className="h-2 w-2 rounded-full bg-cta" />
                    Synced from inbox
                  </div>
                  <h2 className="text-[clamp(32px,4vw,56px)] font-semibold leading-[0.98] tracking-[-0.04em] text-ink">
                    {selectedEmail.subject || "(No subject)"}
                  </h2>
                  <div className="mt-6 grid gap-3 text-sm text-muted">
                    <div className="flex flex-wrap items-center gap-3">
                      <span className="grid h-11 w-11 place-items-center rounded-full bg-wash text-base font-extrabold text-cta">{senderName(selectedEmail.from)[0]?.toUpperCase() ?? "M"}</span>
                      <div className="min-w-0">
                        <strong className="block truncate text-ink">{senderName(selectedEmail.from)}</strong>
                        <span className="block truncate">{senderAddress(selectedEmail.from)}</span>
                      </div>
                    </div>
                    <p className="font-medium">Received {formatFullDate(selectedEmail.date)}</p>
                  </div>
                </div>

                <div className="grid gap-5 py-8 text-[15px] leading-7 text-muted">
                  <p>
                    This portal currently syncs message headers for quick triage. Open webmail to read the full thread, reply, forward, or manage folders.
                  </p>
